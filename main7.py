...
def train_model(train_data_path):
    ...
    learning_rates = [0.001]  # Tasas de aprendizaje
    gammas = [0.95]  # Factores de descuento
    n_steps_list = [10]  # Número de pasos antes de actualizar el modelo
...
def process_data(base_path):
    ...
    for currency_pair in os.listdir(base_path):
        ...
        for timeframe in os.listdir(currency_path):
            ...
            # Iterar a través de las carpetas de datos (train y test)
            for data_type in ["train", "test"]:
                data_path = os.path.join(timeframe_path, data_type)
                            ...
                            for window_size in os.listdir(filter_path):
                                ...
                                for filename in os.listdir(window_path):
                                    if filename.endswith(".csv"):
                                        ...
                                        train_data_path = data_path.replace("test", "train")
                                        train_data_path = train_data_path.replace(filter_type, filter_type)
                                        train_data_path = os.path.join(train_data_path, window_size)
                                        for filename2 in os.listdir(train_data_path):
                                            if filename2.endswith(".csv"):
                                                train_data_path = os.path.join(train_data_path, filename2)
                                        training_folder = os.path.join(os.path.dirname(train_data_path), "entrenamientos", "A2C")
                            ...
                            for window_size in os.listdir(wavelet_path):
                                ...
                                for filename in os.listdir(window_path):
                                    if filename.endswith(".csv"):
                                        ...
                                        if "no_filtrado" in train_data_path:
                                            train_data_path = os.path.join(train_data_path, window_size)
                                        else:
                                            train_data_path = os.path.join(train_data_path, "wavelet", window_size)
                                            
                                        # Find the training data CSV file
                                        train_data_file = None
                                        for filename2 in os.listdir(train_data_path):
                                            if filename2.endswith(".csv"):
                                                train_data_file = os.path.join(train_data_path, filename2)
                                                break
                                        
                                        if train_data_file is None:
                                            print(f"No training data found in {train_data_path}")
                                            continue
