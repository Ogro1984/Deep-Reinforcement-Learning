...
def test_model(model_path, test_data_path, base_path):
    """
    Prueba un modelo A2C con los datos de prueba proporcionados y genera gráficos y estadísticas.

    Args:
        model_path (str): Ruta al modelo entrenado.
        test_data_path (str): Ruta al archivo CSV de datos de prueba.
        base_path (str): Ruta base del proyecto para guardar el archivo CSV de resultados.
    """
    ...
    is_filtered = "filtrado" in test_data_path
    is_normalized = "normalizado" in test_data_path
    start_date = test_df['Close'].index[0] if 'Date' in test_df else 0
    end_date = test_df['Close'].index[-1] if 'Date' in test_df else len(test_df) - 1
    ...
    with open(csv_file, mode='a', newline='') as f:
        ...
        if not file_exists:
            writer.writerow([
                "Model Name", "Learning Rate", "Gamma", "N Steps", "Ent Coef", "VF Coef", "Max Grad Norm", "RMS Prop Eps",
                "Test Data", "Filtered", "Normalized", "Start Date", "End Date",
                "Overall Return", "Final Net Worth", "Buy Operations", "Sell Operations"
            ])
        writer.writerow([
            model_name, params_dict.get("lr_", ""), params_dict.get("gamma_", ""), params_dict.get("nsteps_", ""),
            params_dict.get("ent_", ""), params_dict.get("vf_", ""), params_dict.get("gradnorm_", ""), params_dict.get("rms_", ""),
            test_data_filename, is_filtered, is_normalized, start_date, end_date,
    ...
    ax1.plot(net_worths, label='Patrimonio Neto', color='blue')
    ax1.set_xlabel('Paso de Tiempo')
    ax1.set_ylabel('Patrimonio Neto')
    ax1.set_title(f'Patrimonio Neto a lo Largo del Tiempo - {model_name} (Prueba)')

    # Marcar las acciones en el gráfico del patrimonio neto
    close_prices = test_df['Close'].values
    buy_indices = np.where(np.array(actions) == 1)[0]
    sell_indices = np.where(np.array(actions) == 2)[0]

    ax1.scatter(buy_indices, [net_worths[i] for i in buy_indices], marker='^', color='green', label='Compra')
    ax1.scatter(sell_indices, [net_worths[i] for i in sell_indices], marker='v', color='red', label='Venta')
    ax1.legend()

    # Gráfico de los precios de cierre con las operaciones
    ax2.plot(close_prices, label='Precio de Cierre', color='purple')
    ax2.set_xlabel('Paso de Tiempo')
    ax2.set_ylabel('Precio de Cierre')
    ax2.set_title(f'Precio de Cierre y Operaciones - {model_name} (Prueba)')

    # Marcar las acciones en el gráfico de precios de cierre
    ax2.scatter(buy_indices, [close_prices[i] for i in buy_indices], marker='^', color='green', label='Compra')
    ax2.scatter(sell_indices, [close_prices[i] for i in sell_indices], marker='v', color='red', label='Venta')
    ax2.legend()

    plt.tight_layout()
    plt.savefig(os.path.dirname(model_path) + "/testing_results.png")
    plt.close()
                    ...
                    for window_size in os.listdir(filter_path):
                        ...
                        for filename in os.listdir(window_path):
                            if filename.endswith(".csv"):
                                ...
                                # Construct the training data path based on filter type
                                if "no_filtrado" in filter_path:
                                    train_base_path = os.path.join(train_base_path, window_size)
                                else:
                                    train_base_path = os.path.join(train_base_path, "wavelet", window_size)

                                # Find the training data CSV file
                                train_data_file = None
                                for filename2 in os.listdir(train_base_path):
                                    if filename2.endswith(".csv"):
                                        train_data_file = os.path.join(train_base_path, filename2)
                                        break

                                if train_data_file is None:
                                    print(f"No training data found in {train_base_path}")
                                    continue

                                training_folder = os.path.join(os.path.dirname(train_data_file), "entrenamientos", "A2C")
                                ...
                                # Iterate through the models in the training folder
                                for model_name in os.listdir(training_folder):
                                    model_folder = os.path.join(training_folder, model_name)
                                    model_path = os.path.join(model_folder, "model")
                                    test_model(model_path, test_data_path, base_path)
