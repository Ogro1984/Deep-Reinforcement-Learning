...
def calcular_diferencia_promedio_historico(df, ventana_inicial, pasos, ventana_final, nombre_fichero, porcentaje_entrenamiento):
    ...
    for ventana in range(ventana_inicial, ventana_final + 1, pasos):
        ...
        denoised_hard = pywt.threshold(imfs[-1], np.std(imfs[-1]) * 3, mode='hard')  # Much more aggressive threshold
        ...
        denoised_soft = pywt.threshold(imfs[-1], np.std(imfs[-1]) * 3, mode='soft')  # Much more aggressive threshold
        ...
        denoised_savgol = savgol_filter(imfs[-1], 7, 4)  # Much more aggressive window length and polynomial order
        ...
        # Wavelet denoising
        coeffs = pywt.wavedec(train_data['Close'], 'db1', level=2)
        threshold = np.std(coeffs[-1]) * 3  # Much more aggressive threshold
        denoised_wavelet = pywt.waverec([pywt.threshold(c, threshold, mode='soft') for c in coeffs], 'db1')
        train_data_wavelet = train_data.copy()
        train_data_wavelet['Close'] = denoised_wavelet[:len(train_data_wavelet)]  # Ensure the length matches
        ...
        for data, label in zip([test_data, test_data_normalized],
